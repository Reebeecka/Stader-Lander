...
//Fetch Api through ReadApi function
...
//Fetch Api through ReadWikiApi function
...
export async function readWeatherAync(stad) {
    //Fetches the openweather API corresponding to the city you pressed to get Weather info
    ...
    //Fetches the Wikipedia API corresponding to the city you pressed to get city info  
    let wikiURL = "https://sv.wikipedia.org/w/rest.php/v1/search/page?q=" + cityname + "&limit=1";
    let w = await ReadWikiAPI(wikiURL);

    //Fetches the newsapi API corresponding to the city you pressed to get current news
    ...
    //Fetches Current temparature from weatherAPI
